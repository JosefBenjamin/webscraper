...
public class CrawlLogController {
    ...
    public void startCrawl(Context ctx) {
        ...
        // Step 1: create RUNNING log (short DB transaction)
        Long crawlLogId = scrapeLoggerService.startRunning(sourceId, user.getUsername());

        //Step 2: Execute the crawl and log it
        scrapeLoggerService.executeCrawl(crawlLogId);
        LOGGER.info("User '{}' started crawl for sourceId={}, logId={}", user.getUsername(), sourceId, crawlLogId);

        //Sterp 3: Return status response
        ctx.status(201).json(
                java.util.Map.of("message", "Crawl started", "logId", crawlLogId)
        );
    }
